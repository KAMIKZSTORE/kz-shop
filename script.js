...
function logout() {
  withLoading(() => {
    setCurrent(''); // Hapus current user dari localStorage
    me = null;
    askLogin(); // Tampilkan form login
...
function openProfile(){
  ...
  $('sendOtp').addEventListener('click', ()=>{
    ...
    users[current] = me; saveUsers(users);
    // show loading briefly before showing OTP area
    withLoading(()=> renderOtpArea(true), 700);
    toast('OTP dikirim (simulasi). Kode tampil di UI.');
  });
}
...
function renderOtpArea(show) {
  ...
  if (show && me && me.otpPending) {
    otpArea.innerHTML = `
      <div class="small-muted" style="margin-top:8px">Kode OTP (simulasi): <strong>${me.otpPending.code}</strong></div>
      <div style="display:flex;gap:8px;margin-top:8px">
        <input id="otpCode" class="input" placeholder="Masukkan OTP">
    ...
    $('verifyOtp').addEventListener('click', () => {
      ...
      if(Date.now() > me.otpPending.expires) { delete me.otpPending; users[current]=me; saveUsers(users); renderOtpArea(false); return alert('OTP kadaluarsa.'); }
